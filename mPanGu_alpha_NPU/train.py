...
class LossCallBack(Callback):
    ...
    def step_end(self, run_context):
        ...
        if self._dataset_size > 0 and self.local_rank % 8 == 0:
            ...
            if rank % 8 == 0:
                print(
                    "time: {} local_rank: {}, epoch: {}, step: {}, loss is {}, overflow is {}, scale is {}, lr is {}".  ##lr is {},
                    format(date,
                           int(self.local_rank),
...
def run_train(args_opt):
    ...
    if args_opt.distribute == "true":
        ...
        if args_opt.incremental_training:
            ...
            if rank % 8 == 0:
                ...
                mox.file.copy(f'{args_opt.load_ckpt_obs_path}_word_embedding.npy',
                              f'{args_opt.load_ckpt_local_path}/word_embedding.npy')
                mox.file.copy(f'{args_opt.load_ckpt_obs_path}_position_embedding.npy',
                              f'{args_opt.load_ckpt_local_path}/position_embedding.npy')
                mox.file.copy(f'{args_opt.load_ckpt_obs_path}_top_query_embedding.npy',
    ...
    cache_url = "/cache/Data/"  ##LOCAL_PATH
    if args_opt.offline:
    ...
    if args_opt.optimizer == "lamb":
        ...
        optimizer = FP32StateAdamWeightDecay(group_params, learning_rate=lr, eps=1e-8, beta1=0.99, beta2=0.999)  ## 0.95
    # Initial scaling sens
    loss_scale_value = math.pow(2, 21)  # 12
    ...
    # args_opt.save_checkpoint_bucket_dir = '/home/work/user-job-dir/model/'
...
def restore_checkpoint_from_obs(args_param, sink_size, dataset, model, network, optimizer, epoch):
    ...
    if args_param.pre_trained:
        ...
        restore_bueckt_dir = os.path.join(args_param.load_checkpoint_bucket_dir + f"/rank_{str(rank_id)}",
                                          restore_ckptname)
    ...
    if args_param.has_trained_epoches == 0:
        ...
        params_dict["last_overflow_iterator_step"] = Parameter(Tensor(0, dtype=mstype.int32),
                                                               name="last_overflow_iterator_step")
        params_dict["global_step"] = Parameter(Tensor(np.zeros((1,), dtype=np.int32), dtype=mstype.int32),
                                               name="global_step")
...
def restore_checkpoint_from_obs_64(args_param, sink_size, dataset, model, network, optimizer, epoch):
    ...
    if args_param.pre_trained:
        ...
        restore_bueckt_dir = os.path.join(args_param.load_checkpoint_bucket_dir + f"/rank_{str(rank_id)}",
                                          restore_ckptname)
        restore_bueckt_dir_64 = os.path.join(args_param.load_checkpoint_bucket_dir + f"/rank_{str(rank_id_64)}",
                                             restore_ckptname_64)
    ...
    try:
        ...
        if params_dict:
            if args_param.has_trained_epoches == 0:
                ...
                params_dict["step"] = Parameter(Tensor(0, dtype=mstype.int32), name="step")
                params_dict["scale_sense"] = Parameter(Tensor(65536, dtype=mstype.float32), name="scale_sense")
                params_dict["current_iterator_step"] = Parameter(Tensor(0, dtype=mstype.int32),
