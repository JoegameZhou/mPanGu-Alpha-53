...
class PanguAlphaTrainOneStepWithLossScaleCell(TrainOneStepWithLossScaleCell):
    ...
    def construct(self, input_ids, input_position, attention_mask, layer_past=None, sens=None):
        ...
        if overflow:
            succ = False
            ## lr = self.default_lr
            return F.depend(loss, succ), cond, scaling_sens, self.default_lr
        else:
            succ, lr = self.optimizer(grads)
            ##succ = self.optimizer(grads)
            return F.depend(loss, succ), cond, scaling_sens, lr
...
class PanguAlphaTrainPipelineWithLossScaleCell(nn.Cell):
    ...
    def construct(self,
        ...
        if overflow:
            succ = False
            ret = (loss, overflow, scaling_sens, self.default_lr)
        else:
            succ, lr = self.optimizer(grads)
            ## succ = self.optimizer(grads)
