...
def generate_increment(model, origin_inputs, config, dynamic_generate_length):
    """
    ...
    top_p = config.top_p
    top_k_num = config.top_k_num
    max_generate_length = dynamic_generate_length ## config.max_generate_length
    seq_length = config.seq_length
    end_token = config.end_token
    use_pynative = config.use_pynative_op
...
def generate_increment2(model, origin_inputs, config, dynamic_generate_length):
    """
    Text generation for incremental inference
...
def generate_increment_logits(model, origin_inputs, config, dynamic_generate_length):
    """
    Text generation for incremental inference
