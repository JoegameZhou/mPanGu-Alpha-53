...
def load_train_net(args_opt):
    ...
    lr = LearningRate(learning_rate=args_opt.start_lr, end_learning_rate=args_opt.end_lr,
                      warmup_steps=args_opt.warmup_step, decay_steps=1400)
    params = pangu_alpha_with_loss.trainable_params()
    group_params = set_weight_decay(params)
    if args_opt.optimizer == "lamb":
        optimizer = nn.Lamb(group_params, learning_rate=lr)
    elif args_opt.opt_offload:
        optimizer = AdamWeightDecayOp(group_params, learning_rate=lr, eps=1e-8, beta1=0.9, beta2=0.94,
    ...
    actual_epoch_num = 4#int(epoch_num * step_per_epoch / args_opt.sink_size)
    ...
    update_cell = DynamicLossScaleUpdateCell(loss_scale_value=math.pow(2, 16), scale_factor=2, scale_window=300)
...
def run_train_pipeline(args_opt):
    ...
    loss_scale_value = math.pow(2, 24)
    update_cell = DynamicLossScaleUpdateCell(loss_scale_value=loss_scale_value, scale_factor=1.5, scale_window=300)
...
def run_train(model, net, args_opt, ds, callback, rank_id, epoch_idx):
    step_per_epoch = ds.get_dataset_size()
    callback_size = args_opt.sink_size
    actual_epoch_num = int(0.15*step_per_epoch / callback_size)
    ...
    if epoch_idx != 0:
        ...
        model.train(3, ds, callbacks=callback, sink_size=callback_size, dataset_sink_mode=True)
        # if args_opt.pre_trained:
        #     restore_checkpoint_from_obs_integrated(args_opt, callback_size, ds, pangu_alpha)
...
def main(opt):
    ...
    class Test_Node_MindSpore(fl.client.NumPyClient):
        def __init__(self):
            self.local_ckpt_dir = f"/tmp/ckpt/{rank_id}/device_{rank_id}.ckpt"
            self.count = 0
            if os.path.exists(f"/tmp/ckpt/{rank_id}/"):
                for i in os.listdir(f"/tmp/ckpt/{rank_id}/"):
                    os.remove(f"/tmp/ckpt/{rank_id}/"+i)
            else:
                os.mkdir(f"/tmp/ckpt/{rank_id}/")
        ...
        def fit(self, parameters, config):  # globle model parameters
            ...
            run_train(model, net, args_opt, ds, callback, rank_id, epoch_idx=(self.count - 1))
            return self.get_parameters(), 5, {}

        def evaluate(self, parameters, config):
            self.set_parameters(parameters)
            loss = 1.0  # train(net, train_loader, 1)
            print("======" * 10)
            return float(loss), 5, {"acc: ": float(0.9)}

    if rank_id < 8:
        port = [30000, 30010, 30002, 30003, 30004, 30005, 30006, 30009]
        fl.client.start_numpy_client(f"218.17.115.229:{port[rank_id]}", client=Test_Node_MindSpore(), grpc_max_message_length=2 * 1024 * 1024 * 1024 - 1)
    else:
        for i in range(epoch_num):
            run_train(model, net, args_opt, ds, callback, rank_id, epoch_idx=i)
