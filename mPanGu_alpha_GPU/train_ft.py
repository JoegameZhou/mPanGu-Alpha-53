...
from src.serialization import load_distributed_checkpoint, load_checkpoint, load_param_into_net
...
from src.utils import download_data, get_ckpt_file_list
from src.callbacks import EvalCallBack, LossCallBack
from src.metrics import PPLMetric
from mindspore.common import Parameter
from src.transformer import TransformerOpParallelConfig, CrossEntropyLoss
# from mindspore.nn.transformer import OpParallelConfig as TransformerOpParallelConfig
...
def run_train(args_opt):
    ...
    context.set_context(save_graphs=False, save_graphs_path="./graphs/graphs_of_device_id_" + str(rank))
    ...
    config = PanguAlphaConfig(batch_size=batch_size // micro_batch_interleaved,
                              ...
                              load_ckpt_path=None,#args_opt.load_ckpt_path,
    ...
    ds = create_dataset(config.batch_size * micro_batch_interleaved, data_path=cache_url, data_start_index=0,
                        ...
                        device_num=device_num, rank=rank, column_name=args_opt.data_column_name, epoch=epoch_num, args=args_opt)
    ...
    update_cell = DynamicLossScaleUpdateCell(loss_scale_value=math.pow(2, 24), scale_factor=2, scale_window=1500)
    ...
    if args_opt.incremental_training and "350M" not in args_opt.mode:
        ...
        # #                   range(0, 512)]
        # ckpt_file_list = get_ckpt_file_list("/tmp/ckpt_filter/exp4-2/", device_num=128)
        ...
        parameters["backbone.embedding.position_embedding.embedding_table"] = parameters[
            "backbone.embedding.embedding.position_embedding.embedding_table"]
        ...
        # load_param_into_net(model.train_network, state_dict, strict_load=True)
